...
echo "************** Querying locations API **************"
echo "Retrieving page $page ($size entries starting at $start)..."
    curl -H "Accept: application/hal+json;version=${api_version}" \
         -o taxref_locations_${TAXREFVER}_${start}.json \
         -X GET "https://taxref.mnhn.fr/api/locations?page=${page}&size=${size}"

limit=$(jq 'getpath(["page","totalElements"])' taxref_locations_${TAXREFVER}_${start}.json) 
