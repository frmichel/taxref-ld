...
echo "************** Querying media API **************"
echo "Retrieving page $page ($size entries starting at $start)..."
    curl -H "Accept: application/hal+json;version=${api_version}" \
         -o taxref_media_${TAXREFVER}_${start}.json \
         -X GET "https://taxref.mnhn.fr/api/taxa/media?page=${page}&size=${size}"

limit=$(jq 'getpath(["page","totalElements"])' taxref_media_${TAXREFVER}_${start}.json) 
